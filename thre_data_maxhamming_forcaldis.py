# -*- coding: utf-8 -*-
"""
Created on Wed Jun 20 03:03:42 2018
...
while (threshold <=1):
    ...
    while i < T:
        ...
        simbetween = cal_similarity(store2)
        #1
        disbetween = aveHamming(store2)
#        if len(simbetween) ==0:
#            simbetween.append(0)
        #2
        if len(disbetween) ==0:
       ...
       # print(simbetween)
        ...
        btween.append(sum(simbetween)/len(simbetween))
        #3
        disbtween.append(sum(disbetween)/len(disbetween))
        i=i+1
        #print (averagesim)
    ...
    ave_btween = sum(btween)/len(btween)
    #4
    ave_disbtween = sum(disbtween)/len(disbtween)
    AVEsim=sumsim/len(sim)
    AVEcard=sumcard/len(card)
    #AVEcounting = sumcounting/T
    std_b_num =np.std(belief_num)
    std_btween = np.std(btween)
    #5
    std_disbtween = np.std(disbtween)
...
#        j = j+50
    ...
    avebtw_store.append(ave_btween)
    #6
    disavebtw_store.append(ave_disbtween)
    std_b_num_store.append(std_b_num)
    std_btween_store.append(std_btween)
    #7
    std_disbtween_store.append(std_disbtween)
...
filename ='data_hammax_withbeliefnum50t100aadvance07'+str(T)
path = 'maxthreshold/'
#text_save([thre_store, stdsim_store, stdcard_store,sim_store,card_store,bn_store,counting_store,avebtw_store,std_b_num_store,std_btween_store],path+filename,mode='a')
text_save([thre_store, stdsim_store, stdcard_store,sim_store,card_store,bn_store,counting_store,avebtw_store,std_b_num_store,std_btween_store,disavebtw_store,std_disbtween_store],path+filename,mode='a')
end1 = time.time()
